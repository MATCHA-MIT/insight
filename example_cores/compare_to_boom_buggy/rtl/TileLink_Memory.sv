module tilelink_memory #(
    ...
    parameter DEPTH_WORDS  = 16, // Number of 64-bit words in memory
    ...
    function [ADDR_WIDTH-1:0] word_index(input [ADDR_WIDTH-1:0] base_addr, input [3:0] idx);
        word_index = ((base_addr & 32'h0000_FFFF) >> $clog2(BEAT_BYTES)) + idx;
    endfunction
    ...
    wire addr_violation;
    assign addr_violation = tl_a_valid &&
                    ...
                    if (tl_a_valid && tl_a_ready) begin
                        ...
                        if (DELAY_CYCLES > 0) begin
                            ...
                            if (is_get) begin
                                ...
                                tl_d_data   <= (!is_oob(wi0)) ? mem[wi0] : {DATA_WIDTH{1'b0}};
                                tl_d_denied <= is_oob(wi0);
                            end else begin
                                tl_d_opcode <= 3'b000; // AccessAck for Put
                                tl_d_data   <= {DATA_WIDTH{1'b0}};
                                // Set deny for Put beat 0 based on bounds
                                tl_d_denied <= is_oob(word_index(tl_a_address, 4'd0));
                            end
                            tl_d_valid <= 1'b1;
                            state      <= S_RESP;
                        end
                ...
                S_WAIT: begin
                    ...
                    if (delay_cnt != 16'd0) begin
                        ...
                        if (a_opcode_q == 3'b100) begin
                            ...
                            tl_d_denied <= is_oob(word_index(a_addr_q, 4'd0));
                        end
                        tl_d_valid <= 1'b1;
                        state      <= S_RESP;
                    end
                    ...
                    if (tl_d_valid && tl_d_ready) begin
                        ...
                        if (beat_idx + 4'd1 < beats_total) begin
                            if (a_opcode_q == 3'b100) begin
                                ...
                                tl_d_opcode <= 3'b001;
                                tl_d_data   <= (!is_oob(win)) ? mem[win] : {DATA_WIDTH{1'b0}};
                                tl_d_denied <= is_oob(win);
                            end else begin
                                reg [DATA_WIDTH-1:0] m;
                                reg [ADDR_WIDTH-1:0] win;
