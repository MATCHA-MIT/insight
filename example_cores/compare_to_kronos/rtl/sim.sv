...
// output wire fail
    ...
    reg          illegal_mem_access = 1'b0; // set on illegal memory access
    // assign fail = illegal_mem_access;
    ...
    always @(posedge clk) begin
        // $display("Vincent out of range: dbus %b, ibus %b, dbuss addr %x", dbus_addr_out_of_range, ibus_addr_out_of_range, dbus_addr);
        if (dbus_cyc && dbus_we) begin
                // $display("Vincent Writing to memory at %x, data=%x, be=%b", dbus_addr, dbus_wdata, dbus_be);
