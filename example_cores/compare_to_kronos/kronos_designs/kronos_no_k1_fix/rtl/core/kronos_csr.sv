...
`ifndef MTVEC_RESET_ADDR
`define MTVEC_RESET_ADDR 32'h80000018
`endif
...
localparam logic [31:0] MTVEC_RESET = `MTVEC_RESET_ADDR;
...
always_ff @(posedge clk or negedge rstz) begin
  ...
  else if (csr_illegal_access) begin
    trap_jump <= 1'b1;
...
always_comb begin
  ...
  case(addr)
    ...
    default   : csr_illegal_access = (csr_rd_en || csr_wr_en);
  endcase // addr
  /* verilator lint_on CASEINCOMPLETE */
end
...
// Track whether the retiring instruction is writing to minstret/minstreth
always_ff @(posedge clk or negedge rstz) begin
  if (~rstz)
    minstret_commit_block <= 1'b0;
  else
    minstret_commit_block <= csr_wr_en && (addr == MINSTRET || addr == MINSTRETH);
end
...
kronos_counter64 #(
  ...
  .clk      (clk            ),
  .rstz     (rstz           ),
  .incr     (instret && !minstret_commit_block        ),
  .load_data(csr_wr_data    ),
