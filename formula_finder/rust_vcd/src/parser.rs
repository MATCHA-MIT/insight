...
impl<R: io::BufRead> Parser<R> {
    ...
    fn peek_byte_or_eof(&mut self) -> Option<u8> {
        //let b = io::Read::bytes(&mut self.reader).peekable().peek(); //transpose();
        let b = self.reader.fill_buf().unwrap();
        let r = if b.len() == 0 {
            None
        } else {
            Some(b[0])
    ...
    fn _peek_byte(&mut self) -> Result<u8, io::Error> {
        match self.read_byte_or_eof()? {
            None => Err(io::Error::new(
                io::ErrorKind::UnexpectedEof,
                "unexpected end of VCD file",
    ...
    fn read_token<'a>(&mut self, buf: &'a mut [u8]) -> Result<&'a [u8], io::Error> {
        let mut len = 0;
        let mut identifier_escaped = false;
        loop {
            let b = self.read_byte()?;
            if b == b'\\' && len == 0 {
                identifier_escaped = true;
            }
            let bprime = self.peek_byte_or_eof();
            //println!("read_ybte {} peek byte {:?}", b, bprime);
            if whitespace_byte(b) {
                if len > 0 {
                    if identifier_escaped == false {
                        break;
                    } else if bprime.is_none()  {
                        break;
                    } else if !(whitespace_byte(bprime.unwrap())) {
                        //println!("bprime is not whitespace, breaking!");
                        break;
                    } else if whitespace_byte(bprime.unwrap()) {
                        //break;
                    }
        ...
        //println!("Returning identifier {:?}", &buf[..len]);
        Ok(&buf[..len])
    }
        ...
        loop {
            ...
            if whitespace_byte(b) {
                if r.is_empty() {
                    ...
                    if identifier_escaped == false {
                        break;
                    } else if self.peek_byte_or_eof().is_none() {
                        break;
                    } else if !(whitespace_byte(self.peek_byte_or_eof().unwrap())) {
                        break;
                    } else if whitespace_byte(self.peek_byte_or_eof().unwrap()) {
                        //break;
                    }
    ...
    fn parse_command(&mut self) -> Result<Command, io::Error> {
        ...
        match cmd {
            ...
            b"scope" => {
                ...
                // println!("Read identifier {}", identifier);
                self.read_command_end()?;
